...
class ValidationUtils
{
    ...
    public static function validarContrasena($contrasena) {
        ...
        // Verificar si hay al menos un dígito
        if (!preg_match('/\d/', $contrasena)) {
            return false;
        }

        // Verificar si hay al menos uno de los caracteres #!@*$
    ...
    /**
     * @param $datos array asociatio con nombre_usuario y password
     *
     */
    public static function SaneaUserNameAndPw($datos)
    {
        $datos['nombre_usuario'] = self::sanidarStringFiltro($datos['nombre_usuario']);
        $datos['password'] = self::sanidarStringFiltro($datos['password']);
        return $datos;
    }
